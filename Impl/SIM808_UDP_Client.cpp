...
bool DFRobot_SIM808_UDP_Client::join(const char* apn, const char* userName, const char* passWord)
{
...
void DFRobot_SIM808_UDP_Client::disconnect()
{
    sim808_send_cmd("AT+CIPSHUT\r\n");
}

bool DFRobot_SIM808_UDP_Client::connect(const char* host, int port, int timeout, int chartimeout)
{
    char num[6];
    char resp[96];
...
bool DFRobot_SIM808_UDP_Client::is_connected(void)
{
    char resp[96];
...
bool DFRobot_SIM808_UDP_Client::close()
{
...
int DFRobot_SIM808_UDP_Client::readable(void)
{
    return sim808_check_readable();
}

int DFRobot_SIM808_UDP_Client::wait_readable(int wait_time)
{
    return sim808_wait_readable(wait_time);
}

int DFRobot_SIM808_UDP_Client::wait_writeable(int req_size)
{
    return req_size+1;
}

int DFRobot_SIM808_UDP_Client::send(const char * str, int len)
{
	char num[4];
    if(len > 0){
...
int DFRobot_SIM808_UDP_Client::recv(char* buf, int len)
{
